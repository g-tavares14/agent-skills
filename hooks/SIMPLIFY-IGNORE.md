...
Block-level protection for `/agent-skills:code-simplify`. Mark code that should never be simplified — the model won't see it.
...
| Event | Action |
|---|---|
| `PreToolUse` `read_file` | Backs up file, replaces blocks with `BLOCK_<hash>` placeholders in-place |
| `PostToolUse` `search_replace` / `write` | Expands placeholders back to real code, saves model's changes, re-filters |
...
If a session dies without a Stop hook, files on disk may still have `BLOCK_<hash>` placeholders. To restore manually:

```bash
echo '{}' | bash hooks/simplify-ignore.sh
```

Backups are stored in `.grok/.simplify-ignore-cache/` within the workspace.
...
- **A wholesale rewrite falls back to the backup.** If something replaces a filtered file's entire contents and removes every `BLOCK_<hash>` placeholder, the hook can no longer tell where the protected block belongs. At `Stop` it restores the backup and prints a warning; the rewrite is not lost but is kept in the cache as `.grok/.simplify-ignore-cache/<id>.recovered`, whose path the warning prints, and you merge it back by hand. Changes that leave the placeholders in place — a shell edit, a formatter, an external editor — are kept in the file itself.
