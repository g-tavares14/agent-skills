...
# Parse hook input — Grok uses camelCase (toolName, toolInput, target_file);
# Claude uses snake_case (tool_name, tool_input, file_path). Accept both.
parse_error=""
TOOL_NAME=$(printf '%s' "$INPUT" | jq -r '.toolName // .tool_name // empty' 2>/dev/null) || {
  parse_error="failed to parse tool name from hook input"
  TOOL_NAME=""
}
FILE_PATH=$(printf '%s' "$INPUT" | jq -r '.toolInput.file_path // .toolInput.target_file // .toolInput.path // .tool_input.file_path // .tool_input.target_file // .tool_input.path // empty' 2>/dev/null) || {
  parse_error="failed to parse file path from hook input"
  FILE_PATH=""
}
...
# Map Grok tool names onto the Claude names this script branches on.
case "$TOOL_NAME" in
  read_file) TOOL_NAME="Read" ;;
  search_replace|write|MultiEdit) TOOL_NAME="Write" ;;
esac

# Cache lives with the workspace. Grok plugin hooks set GROK_PLUGIN_ROOT;
# Claude sets CLAUDE_PROJECT_DIR. Prefer Grok when both could be present.
if [ -n "${GROK_PLUGIN_ROOT:-}${GROK_WORKSPACE_ROOT:-}" ]; then
  ROOT="${GROK_WORKSPACE_ROOT:-}"
  [ -z "$ROOT" ] && ROOT=$(printf '%s' "$INPUT" | jq -r '.workspaceRoot // .cwd // empty' 2>/dev/null || true)
  [ -z "$ROOT" ] && ROOT="$PWD"
  CACHE="$ROOT/.grok/.simplify-ignore-cache"
elif [ -n "${CLAUDE_PROJECT_DIR:-}" ]; then
  CACHE="$CLAUDE_PROJECT_DIR/.claude/.simplify-ignore-cache"
else
  ROOT=$(printf '%s' "$INPUT" | jq -r '.workspaceRoot // .cwd // empty' 2>/dev/null || true)
  CACHE="${ROOT:-.}/.grok/.simplify-ignore-cache"
fi
...
# Grok Stop / empty-tool Claude Stop. Do not treat SubagentStop as session-end:
# placeholders must stay on disk while the parent session is still running.
is_stop=0
case "$HOOK_EVENT" in Stop|stop) is_stop=1 ;; esac
if [ -z "$TOOL_NAME" ] && [ -z "$FILE_PATH" ]; then is_stop=1; fi
