...
# ── Test 14: Grok camelCase payload uses .grok cache ────────────────────
printf '\nTest 14: Grok read_file / search_replace envelope\n'

if ! command -v jq >/dev/null 2>&1; then
  printf '  SKIP: full lifecycle needs jq (the hook exits at its jq guard)\n'
else
  PROJ="$TMPDIR/proj-grok"
  rm -rf "$PROJ"
  mkdir -p "$PROJ/.grok"
  TARGET="$PROJ/app.ts"
  cat > "$TARGET" <<'EOF'
...
EOF
  ...
  hook_event() { printf '%s' "$1" | GROK_PLUGIN_ROOT="$PWD" GROK_WORKSPACE_ROOT="$PROJ" bash hooks/simplify-ignore.sh; }
  read_event=$(printf '{"hookEventName":"pre_tool_use","hook_event_name":"PreToolUse","toolName":"read_file","toolInput":{"target_file":"%s"},"workspaceRoot":"%s"}' "$TARGET" "$PROJ")
  edit_event=$(printf '{"hookEventName":"post_tool_use","hook_event_name":"PostToolUse","toolName":"search_replace","toolInput":{"file_path":"%s"},"workspaceRoot":"%s"}' "$TARGET" "$PROJ")
  stop_event=$(printf '{"hookEventName":"stop","hook_event_name":"Stop","workspaceRoot":"%s"}' "$PROJ")

  hook_event "$read_event"
  assert_eq "Grok Read hides protected block" "1" "$(grep -c 'BLOCK_' "$TARGET")"
  assert_eq "Grok cache is under .grok" "1" "$([ -d "$PROJ/.grok/.simplify-ignore-cache" ] && echo 1 || echo 0)"
  assert_eq "Claude cache not used on Grok" "0" "$([ -d "$PROJ/.claude/.simplify-ignore-cache" ] && echo 1 || echo 0)"

  printf 'const added = 3;\n' >> "$TARGET"
  hook_event "$edit_event"
  hook_event "$stop_event"

  assert_eq "Grok Stop restores protected block" "1" "$(grep -c 'const protectedValue = 42;' "$TARGET")"
  assert_eq "Grok Stop keeps model edit" "1" "$(grep -c 'const added = 3;' "$TARGET")"
  assert_eq "no placeholder after Grok Stop" "0" "$(grep -c 'BLOCK_' "$TARGET")"
fi
