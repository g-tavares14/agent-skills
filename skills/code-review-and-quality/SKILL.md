...
4. Does it have known vulnerabilities? (`npm audit` or `pip-audit`)
...
For triaging `npm audit` / `pip-audit` findings and supply-chain risk (typosquatting, compromised maintainers), follow the `security-and-hardening` skill — this section covers the upgrade *workflow*, that one covers the security verdict.
