...
This is not `/agent-skills:review`. `/agent-skills:review` is a verdict on a finished artifact. This is an in-flight posture: non-trivial decisions get cross-examined while course-correction is still cheap.
...
| "I'll do doubt at the end with `/agent-skills:review`" | `/agent-skills:review` is a final gate. Doubt-driven catches wrong directions early when course-correction is cheap. By PR time it's too late. |
...
- Doubting only after committing — that's `/agent-skills:review`, not doubt-driven development
...
- **`code-review-and-quality` / `/agent-skills:review`**: complementary. `/agent-skills:review` is post-hoc PR verdict; doubt-driven is in-flight per-decision. Use both.
