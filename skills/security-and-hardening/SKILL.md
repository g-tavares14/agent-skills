...
Do not assume npm (or pip) or treat the nearest manifest as the install root. Apply this order:
...
- **Never apply forced audit remediation automatically** (`npm audit fix --force`, unreviewed `pip install --upgrade`, or equivalent). Preview the remediation, read changelogs, and test each resulting upgrade; forced fixes may cross declared dependency ranges.
