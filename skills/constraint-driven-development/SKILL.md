...
| What | Where to look |
|------|---------------|
| Language and stack | `package.json`, `pyproject.toml` |
...
| Coverage (TypeScript) | your test runner | already there | `vitest run --coverage` (or `jest --coverage`) | coverage of changed lines |
...
| Assertion quality (TS) | Stryker | `npm i -D @stryker-mutator/core` | `stryker run --mutate <changed files>` | mutation score |
| Assertion quality (Python) | mutmut | `pip install mutmut` | `mutmut run --paths-to-mutate <changed files>` | mutation score |
...
```toml
[project.scripts]
# or a Makefile / justfile — the names matter more than the file
# check:fast  → ruff check . && mypy . && gitleaks detect --redact --no-banner
# check:task  → check:fast && pytest --cov
# check:full  → check:task && semgrep scan --config p/default && osv-scanner scan source -r .
```
...
2. **Scripted.** An `npm run check` or `pytest`/script entry in `pyproject.toml` that runs the fast checks, wired into your agent's post-edit hook and your CI. Deterministic, no new dependency.
