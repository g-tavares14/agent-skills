...
## Tech Stack
- TypeScript: React 18, TypeScript 5, Vite, Node.js 22
- Python: FastAPI, Python 3.12, pytest, ruff, mypy
- PostgreSQL

## Commands
TypeScript: `npm run build` · `npm test` · `npm run lint --fix` · `npx tsc --noEmit` · `npm run dev`
Python: `pytest` · `ruff check .` · `ruff format .` · `mypy .` · `uvicorn app.main:app --reload`

## Code Conventions
- TypeScript: functional components, named exports, tests next to source (`Button.tsx` → `Button.test.tsx`)
- Python: type hints on public functions, pytest naming `test_*.py`, ruff for lint/format
