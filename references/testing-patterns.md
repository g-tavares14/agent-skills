# Testing Patterns Reference (TypeScript and Python)

Quick reference of testing patterns in **TypeScript** (Vitest/Jest, React Testing Library, Supertest, Playwright) and **Python** (pytest). These illustrate the universal principles from the `test-driven-development` skill. Use the stack of the repository you are in.
...
class TestTaskServiceCreateTask:
    def test_creates_a_task_with_default_pending_status(self):
        ...
...
# Async
@pytest.mark.asyncio
async def test_saves_the_task():
    task = await create_task({"title": "Test"})
    assert task.id is not None
```

```python
# Mock at boundaries only (httpx, DB, filesystem)
def test_returns_cached_user(mocker):
    mocker.patch("app.users.fetch_remote", return_value={"id": "1", "name": "Ada"})
    user = get_user("1")
    assert user.name == "Ada"
```

```python
# API / integration (FastAPI + httpx)
from fastapi.testclient import TestClient
from app.main import app

client = TestClient(app)

def test_creates_a_task_and_returns_201():
...
def test_returns_422_for_invalid_input():
    response = client.post("/api/tasks", json={"title": ""})
    assert response.status_code == 422
```

```python
# E2E (Playwright for Python) — same user-flow discipline as the TypeScript example
from playwright.sync_api import Page, expect

def test_user_can_create_and_complete_a_task(page: Page):
    page.goto("/")
    page.get_by_role("textbox", name="email").fill("test@example.com")
    page.get_by_label("password").fill("testpass123")
    page.get_by_role("button", name="log in").click()
    page.get_by_role("button", name="new task").click()
    page.get_by_role("textbox", name="title").fill("Buy groceries")
    page.get_by_role("button", name="create").click()
    task = page.get_by_role("listitem", name="buy groceries")
