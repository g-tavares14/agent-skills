...
- **Invoke via:** `/agent-skills:webperf` (dedicated performance audit command). Not included in `/agent-skills:ship` fan-out — performance audits apply to web applications only, not to utility libraries or CLI tools, so adding it to a global pre-launch fan-out would create noise in non-web projects.
