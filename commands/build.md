...
when-to-use: Use when the user runs /agent-skills:build or asks for this lifecycle step.
---
...
- `/agent-skills:build` — implement the next pending task, then stop (careful, one slice at a time).
- `/agent-skills:build auto` — generate the plan if needed, get a single approval, then implement every task without stopping between them.
...
## Autonomous: the whole plan (`/agent-skills:build auto`)
