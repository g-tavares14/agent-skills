...
when-to-use: Use when the user runs /agent-skills:ship or asks for this lifecycle step.
---

Invoke the shipping-and-launch skill.

`/agent-skills:ship` is a **fan-out orchestrator**. It runs three specialist personas in parallel against the current change, then merges their reports into a single go/no-go decision with a rollback plan. The personas operate independently — no shared state, no ordering — which is what makes parallel execution safe and useful here.

## Phase A — Parallel fan-out

Spawn three subagents concurrently with `spawn_subagent`. Set `subagent_type` to `code-reviewer`, `security-auditor`, and `test-engineer` when those types exist; if this pack is installed as a plugin they are `agent-skills:code-reviewer`, `agent-skills:security-auditor`, and `agent-skills:test-engineer`. If neither form is available, use `general-purpose` and prepend the matching file from `agents/`. **Issue all three `spawn_subagent` calls in a single assistant turn so they execute in parallel** — sequential calls defeat the purpose of this command.
...
If subagents are unavailable, invoke each persona's system prompt sequentially in the main context and treat their outputs as if returned in parallel — the merge phase still works.
...
**Persona resolution.** If you've defined your own `code-reviewer`, `security-auditor`, or `test-engineer` in `agents/` or your global configuration, those take precedence over this plugin's versions — `/agent-skills:ship` picks up your customizations automatically.
...
5. **Skip the fan-out only if all of the following are true:** the change touches 2 files or fewer, the diff is under 50 lines, and it does not touch auth, payments, data access, or config/env. Otherwise, default to fan-out. `/agent-skills:ship` is designed for production-bound changes — when the blast radius is non-trivial, run the parallel review even if the diff looks small.
