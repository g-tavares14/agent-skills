...
when-to-use: Use when the user runs /agent-skills:webperf or asks for this lifecycle step.
---

/agent-skills:webperf targets web applications specifically. Do not use it for utility libraries, CLIs, or server-only code with no browser-facing output.
...
Spawn the `web-performance-auditor` subagent with `spawn_subagent`. Prefer `subagent_type: web-performance-auditor`, then `agent-skills:web-performance-auditor`; otherwise `general-purpose` with `agents/web-performance-auditor.md` prepended. Pass it explicitly:
