...
- Default — implement the next pending task, then stop (one slice at a time).
- `auto` (or `all`) — generate the plan if needed, get a single approval, then implement every task without stopping between them.

Treat `auto` or `all` in the user message as autonomous mode; anything else (or empty) is the default single-task mode. Autonomous mode is not faster per task — it runs the same test-driven loop — it only removes the human stepping between tasks.
...
## Autonomous: the whole plan (`auto`)
